...
export function useReportMetrics(userId, branchId) {
  ...
  const run = useCallback(async () => {
    ...
    metrics.push(metric('إجمالي المبيعات', fmt(cmSales) + ' ر.س', pct(cmSales, pmSales), 'moreGood'));
    metrics.push(metric('متوسط الفاتورة', fmt(cmAvg) + ' ر.س', pct(cmAvg, pmAvg), 'moreGood'));
