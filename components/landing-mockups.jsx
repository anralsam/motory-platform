...
function Plate({ value = 'أ ب ج 1234' }) {
  return (
    <div className="flex items-stretch overflow-hidden rounded-md border-2 border-gray-800 bg-white font-mono" dir="ltr">
      ...
      <span className="px-2 py-1 text-sm font-extrabold tracking-widest text-slate-900">{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div className="text-sm font-extrabold text-slate-900">إنشاء طلب جديد</div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {['رقم الجوال', 'اسم العميل', 'الشركة', 'الطراز'].map((l, i) => (
          <div key={i}>
            <div className="mb-1 text-[11px] font-bold text-slate-500">{l}</div>
            <div className="h-8 rounded-lg border border-slate-200 bg-slate-50" />
          </div>
        ))}
      </div>
      <div className="mt-3">
        <div className="mb-1 text-[11px] font-bold text-slate-500">رقم اللوحة</div>
        <Plate />
      </div>
      <div className="mt-3 flex flex-wrap gap-1.5">
        {['غسيل VIP', 'تلميع', 'تنظيف داخلي'].map((s, i) => (
          <span key={i} className={`rounded-full border px-2.5 py-1 text-[11px] font-bold ${i === 0 ? 'border-brand bg-brand text-white' : 'border-slate-200 text-slate-600'}`}>{s}</span>
        ))}
      </div>
...
export function KanbanMock() {
  ...
  return (
    <div className="grid w-full max-w-md grid-cols-3 gap-2.5">
      {cols.map((col, i) => (
        <div key={i} className="rounded-2xl bg-slate-100/70 p-2">
          ...
          {col.cards.map((c, k) => (
            <div key={k} className="rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm">
              <div className="mb-1.5 truncate text-[12px] font-extrabold text-slate-900">{c.c}</div>
    ...
    <div className="w-full max-w-sm space-y-2.5">
      {cards.map((c, i) => (
        <div key={i} className={`flex items-start gap-3 rounded-2xl border p-3 ${c.tone}`}>
          ...
          <div>
            <div className="text-[13px] font-extrabold text-slate-900">{c.t}</div>
            <div className="mt-0.5 text-[12px] leading-snug text-slate-600">{c.b}</div>
          </div>
        </div>
      ))}
    </div>
...
export function InvoiceMock() {
  ...
  return (
    <div className="w-[230px] rounded-2xl bg-white p-4 font-mono text-[11px] leading-tight text-black shadow-lg">
      ...
      <div className="my-1.5 text-center tracking-tighter text-slate-400">{dash}</div>
      {[['رقم الفاتورة', 'INV-9A1F'], ['التاريخ', '23/06/2026'], ['العميل', 'سعد العتيبي']].map(([k, v], i) => (
        <div key={i} className="flex justify-between"><span className="text-slate-600">{k}</span><span className="font-bold">{v}</span></div>
      ))}
      <div className="my-1.5 text-center tracking-tighter text-slate-400">{dash}</div>
      <div className="flex justify-between"><span>غسيل VIP</span><span>90.00</span></div>
      <div className="flex justify-between text-slate-600"><span>ض.ق.م 15%</span><span>13.50</span></div>
      <div className="mt-1 flex justify-between text-[13px] font-extrabold"><span>الإجمالي</span><span>103.50</span></div>
      <div className="my-1.5 text-center tracking-tighter text-slate-400">{dash}</div>
      <div className="flex flex-col items-center gap-1 py-1">
        <svg width="56" height="56" viewBox="0 0 24 24" className="text-slate-900"><path fill="currentColor" d="M3 3h8v8H3V3zm2 2v4h4V5H5zm8-2h8v8h-8V3zm2 2v4h4V5h-4zM3 13h8v8H3v-8zm2 2v4h4v-4H5zm10 0h2v2h-2v-2zm4 0h2v2h-2v-2zm-4 4h2v2h-2v-2zm4 0h2v2h-2v-2zm-2-2h2v2h-2v-2z" /></svg>
        <div className="text-[9px] text-slate-500">رمز ZATCA</div>
      </div>
    </div>
  );
}
