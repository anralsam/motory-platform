...
export const getIntelligenceData = unstable_cache(
  async () => {
    ...
    const [{ data: orders }, { data: workers }, { data: branches }] = await Promise.all([
      admin.from('orders').select('id, status, branch_id, assigned_to, created_at, started_at, completed_at, price, customer_name'),
...
export async function getWorkerData(userId) {
  ...
  const [{ data: orders }, inv, svc] = await Promise.all([
    admin.from('orders')
  ...
  return { orders: orders || [], inventory: inv?.data || [], services: svc?.data || [] };
}
