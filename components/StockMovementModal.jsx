    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose?.(); }}>
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-1 flex items-center justify-between">
          <h3 className="text-lg font-extrabold text-slate-900">تسجيل حركة مخزون</h3>
        ...
        <p className="mb-4 text-sm text-slate-500">
          {item.name} — المتوفر حالياً: <span className="font-extrabold text-slate-800">{item.qty} {item.unit}</span>
        </p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`flex items-center justify-center gap-2 rounded-xl border-2 px-3 py-3 text-sm font-extrabold transition ${
              type === 'in' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500 hover:border-slate-200'
            }`}
          >
          ...
          <button
            ...
            onClick={() => setType('out')}
            className={`flex items-center justify-center gap-2 rounded-xl border-2 px-3 py-3 text-sm font-extrabold transition ${
              type === 'out' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-500 hover:border-slate-200'
            }`}
          >
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-bold text-slate-600">السبب / الملاحظة</span>
            <input
          ...
          {n > 0 && (
            <div className="rounded-xl bg-slate-50 px-3.5 py-3 text-sm font-bold text-slate-600">
              الكمية بعد الحركة: <span className={`font-extrabold ${type === 'out' && projected < item.min ? 'text-red-600' : 'text-slate-900'}`}>{projected} {item.unit}</span>
            </div>
          )}
        </div>
