...
export function useDashboard(centerId, branchId) {
  ...
  const run = useCallback(async () => {
    ...
    // Raw orders (≤1y) → feeds the UnifiedChart matrix (revenue/sales/customers × day/week/month/year).
    const ordersRawP = onB(
      supabase.from('orders')
        .select('id, created_at, status, price, service_type, assigned_to, customer_name')
    ...
    const [{ data: visits }, { data: ordersToday }, { count: customers }, { data: recent }, { data: workers }, { data: ordersRaw }] =
      await Promise.all([visitsP, ordersTodayP, custQ, recentP, workersP, ordersRawP]);
