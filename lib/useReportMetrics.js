...
import { computeEfficiency } from '@/components/dashboard-pro/dna/engine';
...
export function useReportMetrics(userId, branchId) {
  const [state, setState] = useState({ loading: true, metrics: [], series: [], durations: null });

  const run = useCallback(async () => {
    if (!userId) { setState({ loading: false, metrics: [], series: [], durations: null }); return; }
    ...
    // Stage durations from orders — the single engine computes production
    // (ready−started) and handover (completed−ready), skipping missing stamps.
    let oq = supabase
      .from('orders')
      .select('started_at,ready_at,completed_at,created_at')
      .eq('merchant_id', userId)
      .gte('created_at', pmStart.toISOString());
    ...
    const cmEff = computeEfficiency(cmOrders);
    const pmEff = computeEfficiency(pmOrders);
    // The headline "متوسط مدة العمل" card now tracks PRODUCTION speed (the number
    // that actually populates), not the old completed−started that stayed empty.
    const cmDur = cmEff.avgProduction, pmDur = pmEff.avgProduction;
    const durations = { production: cmEff.avgProduction, handover: cmEff.avgHandover, lifecycle: cmEff.avgLifecycle, sample: cmEff.producedCount };
    ...
    if (cmDur != null) {
      metrics.push(metric('متوسط زمن العمل الفعلي', cmDur + ' دقيقة', pmDur != null ? pct(cmDur, pmDur) : 0, 'lessGood'));
    }
    ...
    setState({ loading: false, metrics, series, durations });
  }, [userId, branchId]);
