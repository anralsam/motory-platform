 ...
 */
export async function logWorkerActivity(admin, { workerId, merchantId, actionType, description, durationMin = null }) {
  ...
  try {
    await admin.from('worker_activities').insert({
      ...
      duration_min: (durationMin != null && Number.isFinite(durationMin) && durationMin >= 0) ? Math.round(durationMin) : null,
    });
  } catch {
