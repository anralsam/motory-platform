...
export function useDashboard(centerId, branchId) {
  ...
  // ── Full matrix — once per center (branch filtering happens client-side) ──
  useEffect(() => {
    if (!centerId) { setOrders([]); return undefined; }
    let alive = true;
    (async () => {
      const now = new Date();
      const yearStart = new Date(now); yearStart.setDate(now.getDate() - 366); yearStart.setHours(0, 0, 0, 0);
      const { data } = await supabase.from('orders')
        .select('id, created_at, status, price, service_type, assigned_to, customer_name, branch_id')
  ...
  return { ...state, orders, refetch: run };
}
